...
func init_block_lights():
	for x in range(light_values.size()):
		for y in range(light_values[x].size()):
			if get_node("../TileMap").get_cell(x, y) == -1:
				light_block(x, y, 1, 0)
...
func set_size():
	...
	size *= get_node("../CharacterBody2D/Camera2D").zoom
	size.x = snapped(size.x, 64)
	size.y = snapped(size.y, 64)

	size_in_tiles.x = int(size.x / 32)
	size_in_tiles.y = int(size.y / 32)

	scale = size

func set_pos():
	var pos_x = snapped(get_node("../CharacterBody2D").position.x, snap_step) - size.x / 2
	var pos_y = snapped(get_node("../CharacterBody2D").position.y, snap_step) - size.y / 2
...
func set_shader_tiles():
	...
	var start_x = int(position.x / 32)
	var start_y = int(position.y / 32)
	...
	material.set_shader_parameter("light_values", tex)
